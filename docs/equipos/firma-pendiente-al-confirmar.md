...
>
> **Revisión 3 (07/09/2026):** cada firma actualiza la responsiva **de inmediato**, no solo cuando se completan las dos. Antes, alguien que ya había firmado su parte no la veía reflejada en la carta hasta que la otra persona también firmara — confuso ("ya firmé pero no sale mi firma"). Ver "La regla" y §`firmas_completas` más abajo.
...
- **Una firma ya capturada no se puede volver a subir** — es evidencia. El endpoint responde `409 TRANSICION_INVALIDA`. (Las fotos de entrega, en cambio, sí son reemplazables en cualquier momento del ciclo — ver `docs/equipos/fotos-entrega-reemplazables.md`.)
...
- `backend/tests/equipos/test_api_prestamos.py`: confirmar sin ninguna firma procede igual; una firma no se puede subir antes de confirmar; **solo el titular** puede subir `firma_entrega` (otro `APROBADOR_EQUIPO` sin el paquete singleton recibe 403 igual que quien llena el formulario); un aprobador puro (sin `equipos_prestamos:solicitar`, con `APROBADOR_EQUIPO` + `TITULAR_FIRMA_EQUIPO`) no puede subir fotos ni `firma_responsable` pero sí `firma_entrega`; **cada firma genera su propia versión de responsiva** (v2 con la primera, v3 con la segunda — revisión 3); no se puede resubir una firma ya capturada; la protección es por `(prestamo, kind)`, no global.
- `backend/tests/equipos/test_aprobacion.py`: `confirmar-devolucion` y `cerrar-incidencia` rechazan con cualquiera de las dos firmas pendiente (incluso con entrega ya autorizada) y no escriben nada; completarlas desbloquea; la bitácora registra el ciclo completo con **dos** pares `firma_completada`/`responsiva_generada`, uno por firma.
- `backend/tests/equipos/test_responsiva_pdf.py`: el nombre bajo cada firma es quien realmente la subió, no un campo fijo del préstamo; el titular aparece por default antes de que exista una firma real; cambiar de titular después de una firma real no la reescribe con efecto retroactivo; la v2 (tras la primera firma) sigue diciendo "FIRMA PENDIENTE" solo para la que falta, la v3 (tras la segunda) ya no dice nada.
- `backend/tests/equipos/test_notificaciones.py`: el aviso de confirmación menciona las firmas pendientes; **cada** firma avisa por correo de inmediato con la responsiva actualizada adjunta (no solo la última); un reintento de un correo de firma con sufijo `:{kind}` conserva el adjunto (regresión: la idempotencia de `notification_log` lo comparaba contra el tipo completo, no el base).
