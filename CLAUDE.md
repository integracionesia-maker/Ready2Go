...
- **Gastos generales (R12)** — tabla `general_expenses` independiente de `tickets`: sin `creator_id`, sin `budget_cycle_id`, sin `status` de validación, pero **con `brand_id` obligatorio** (`nullable=False` — todo gasto general debe estar asociado a una marca para trazabilidad y reportes por marca). Se crean y cuentan de inmediato, solo `admin`/`superadmin`. Mismo patrón de soft/hard delete que tickets pero sin reversión de ciclo (no aplica). Detalle completo: `docs/presupuestos/gastos-generales-manual.md`.
- **Gastos Operativos (módulo nuevo, hermano de Presupuestos/Equipos)** — acumulador de gastos **aislado de marketing**: tablas propias `expense_rubros` (catálogo de rubros, editable) y `operational_expenses`; sin ciclo, sin validación, sin límite. Cada gasto tiene **dos fechas**: `fecha_gasto` (manual, **define el mes** para dashboard y export) y `upload_date` (automática, trazabilidad). Comprobante **obligatorio**; **solo borrado lógico** (no hay físico). Módulo RBAC `gastos_operativos` (acciones `ver`/`crear`/`borrar`/`exportar`/`gestionar_rubros`); acceso: `admin`, `superadmin`, `marketing_admin` y el rol base nuevo `operativo` (que **no** ve Presupuestos ni Equipos); el resto de marketing (`marketing_presupuestos`/`marketing_equipos`/`marketing_basico`) y `creador` **no** lo ven. Sus totales nunca entran en dashboards/KPIs/export de marketing. Endpoints `/api/rubros` y `/api/operational-expenses`. Detalle: `docs/gastos-operativos/plan-implementacion.md` y `docs/gastos-operativos/manual-usuario.md`.
